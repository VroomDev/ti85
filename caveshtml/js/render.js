...
export function createRenderer(canvas, pack) {
  ...
  function drawMini(data, engine) {
    const cell = 2;
    const view = LEVEL_W;
    const origin = wrapMap(engine.player.xy - (view >> 1) - (view >> 1) * LEVEL_W);
    for (let y = 0; y < view; y++) {
      for (let x = 0; x < view; x++) {
        const tile = engine.map[wrapMap(origin + y * LEVEL_W + x)];
        fillRect(data, x * cell, y * cell, cell, cell, colorForTile(tile));
      }
    }
    const cx = (view >> 1) * cell;
    const cy = (view >> 1) * cell;
    fillRect(data, cx, cy, cell, cell, colorForTile(playerid));
  }
